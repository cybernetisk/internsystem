...
class Råvare(models.Model):
    STATUS_CHOICES = (("OK", "I bruk"), ("OLD", "Utgått"))
    ...
    innkjopskonto = models.ForeignKey(
        Konto, related_name="raavarer", on_delete=models.CASCADE
    )

    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default="OK")
    lenket_salgsvare = models.ForeignKey(
        "Salgsvare",
        related_name="lenkede_raavarer",
        null=True,
        blank=True,
...
class Råvarepris(models.Model):
    ...
    aktiv = models.BooleanField(
        default=True, help_text="Hvorvidt dette er/har vært en reell råvarepris for oss"
    )
...
class Salgsvare(models.Model):
    kategori = models.CharField(max_length=50, null=True, blank=True)
    navn = models.CharField(max_length=100)
    salgskonto = models.ForeignKey(
        Konto, related_name="salgsvarer", on_delete=models.CASCADE
    )
    status = models.CharField(
        max_length=10, choices=Råvare.STATUS_CHOICES, default="OK"
    )
    kassenr = models.PositiveSmallIntegerField(
        help_text="Nr i varekatalog i kassa", null=True, blank=True
    )
    kassenavn = models.CharField(
        max_length=15, help_text="Navn i varekatalog i kassa", null=True, blank=True
    )
    raavarer = models.ManyToManyField(
        Råvare, through="varer.SalgsvareRåvare", related_name="salgsvarer"
    )
...
class SalgsvarePris(models.Model):
    ...
    salgsvare = models.ForeignKey(
        Salgsvare, related_name="priser", on_delete=models.CASCADE
    )
    status = models.CharField(max_length=3, choices=STATUS_CHOICES, default="FOR")
    dato = models.DateField()
    mva = models.PositiveSmallIntegerField(default="25")
    pris_intern = models.PositiveSmallIntegerField(
        help_text="Internpris INK mva", null=True, blank=True
    )
    pris_ekstern = models.PositiveSmallIntegerField(
        help_text="Eksternpris INK mva", null=True, blank=True
    )
    ...
    def __str__(self):
        return "%s (%s - %s) %s / %s" % (
            ...
            self.dato.isoformat(),
            self.status,
            ("kr %g" % self.pris_intern)
            if self.pris_intern is not None
            else "se ekstern",
            ("kr %g" % self.pris_ekstern)
            if self.pris_ekstern is not None
            else "ikke salg",
        )
...
class Salgskalkyle(models.Model):
    ...
    kommentar = models.TextField(null=True, blank=True)
    dato = models.DateField()
    varer = models.ManyToManyField(
        Salgsvare, through="varer.SalgskalkyleVare", related_name="salgskalkyler"
    )
...
class SalgskalkyleVare(models.Model):
    ...
    salgsvare = models.ForeignKey(Salgsvare, on_delete=models.CASCADE)
    interngrad = models.FloatField(
        null=True,
        blank=True,
        help_text="Prosent andel (antall enheter) solgt til internpris",
    )
    antall = models.PositiveIntegerField()
...
class Varetelling(models.Model):
    ...
    varer = models.ManyToManyField(
        Råvare, through="varer.VaretellingVare", related_name="varetellinger"
    )
    is_locked = models.BooleanField(
        default=False, help_text="Sperr tellingen for endringer"
    )
...
class VaretellingVare(models.Model):
    ...
    raavare = models.ForeignKey(Råvare, on_delete=models.CASCADE)
    time_price = models.DateField(
        null=True, blank=True, help_text="Overstyring av tidspunkt varen skal prises"
    )
    added_by = models.ForeignKey(
        User,
        editable=False,
        null=True,
        help_text="Brukeren som registrerte oppføringen",
        on_delete=models.CASCADE,
    )
    time_added = models.DateTimeField(auto_now_add=True)
    sted = models.CharField(max_length=50, null=True, blank=True)
    antall = models.FloatField()
    antallpant = models.FloatField(
        help_text="Antall hele forpakninger det skal telles pant for, brukes vanlig antall (avrundet opp) hvis ikke spesifisert",
        null=True,
        blank=True,
    )
