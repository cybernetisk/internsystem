...
class FixturesTestCase(TestCase):
    ...
    def test_semester_fixtures(self):
        self.assertEqual(
            Member.objects.count(), 5, "There are five members registererd"
        )
        self.assertEqual(
            Member.objects.filter(honorary=True).count(),
            1,
            "There are one honary member",
        )
        self.assertEqual(
            Member.objects.filter(lifetime=True).count(),
            1,
            "There are one lifetime member",
        )
...
class MemberRestTestCase(APITestCase):
    ...
    def login(self):
        user = User.objects.get_or_create(
            username="cyb", email="text@example.com", is_superuser=True
        )
